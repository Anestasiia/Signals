...
def compute_fourier_coefficient(k, f, N):
    A_k, B_k = compute_fourier_term(k, f, N) #список пар коецієнтів
    return A_k + 1j * B_k
...
n = 113  # 96 + 17 - кількість точок сигналу
N = 8 # елементи сигналу
N1 = 64
f = [1, 1, 1, 1, 0, 0, 0, 1]  # число 113 у двійковій системі
f1 = [random.choice([0, 1]) for _ in range(N1)]

# кількість операцій додавання та множення
dft_add = N * (N - 1) # кільксть додавань в дискретному перетворенні
dft_multi = N ** 2
